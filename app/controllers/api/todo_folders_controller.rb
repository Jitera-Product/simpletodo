...
module Api
  class TodoFoldersController < Api::BaseController
    before_action :doorkeeper_authorize!, only: [:create, :abort, :resolve_conflict]
    ...
    def resolve_conflict
      ...
      # Validation for user_id and folder_name
      if user_id.blank?
        ...
        render json: { error: 'Folder name cannot exceed 255 characters' }, status: :unprocessable_entity
        return
      end
      ...
      if uniqueness_check[:error]
        ...
        if result.success?
          render json: { status: 200, todo_folder: result.as_json(only: [:id, :user_id, :name, :created_at, :updated_at]) }, status: :ok
        else
